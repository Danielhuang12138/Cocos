...
Ubullet* Ubullet::create(Vec2 position, float speed, int dir)
{
	Ubullet* pRet = new(std::nothrow) Ubullet();
	if (pRet && pRet->init(position, speed, dir))
	{
		pRet->autorelease();
		return pRet;
...
bool Ubullet::init(Vec2 position, float speed, int dir)
{
	if (!BaseObject::init())
	{
		return false;
	}
	m_texture = Director::getInstance()->getTextureCache()->addImage("Chapter12/tank/ubullet.png");
	switch (dir)
	{
	case BULLET_UP:
		m_sprite = Sprite::createWithTexture(m_texture, Rect(0, 0, 8, 8));
...
void Ubullet::update(float delta)
{
	m_rect = Rect(getPositionX() - 4, getPositionY() - 4, 8, 8); // 更新rect
	if (this->getLife() <= 0)
	{
...
void Ubullet::Blast()
{
	this->setVisible(false);   // 子弹消失
	setLife(0);
	auto explode = Sprite::create("Chapter12/tank/uexplode1.png");
	this->getParent()->addChild(explode, 10);
	explode->setPosition(this->getPosition());  // 显示爆炸
	explode->runAction(Sequence::create(
		DelayTime::create(0.1f),
		FadeOut::create(0.1f),                   // 爆炸消失
		CallFunc::create(CC_CALLBACK_0(Ubullet::deleteObj, this, explode)),
		NULL
	));
}

void Ubullet::deleteObj(Sprite* obj)
{
	obj->removeFromParent();
	this->removeFromParent();
}
